...
{
    public class ServerPatchRequester
    {
        ...
        public ServerPatchRequester(string apiAddress)
        {
            _apiAddress = apiAddress;
        }
        
        public void EnqueuePatchRequest(ServerRequestData data)
        {
        ...
        private async void ProcessQueue()
        {
            ...
            {
                await SendPatchRequest(data);
            }

            _isProcessing = false;
        }

        private async Task SendPatchRequest(ServerRequestData requestData)
        {
            using UnityWebRequest request = new UnityWebRequest(_apiAddress + requestData.Data, "PATCH");
...
#if UNITY_EDITOR
            Debug.Log($"Send PATCH request: URI: [{request.uri.AbsoluteUri}]");
#endif
            ...
            {
                Debug.Log($"Request PATCH error: URI: [{request.uri.AbsoluteUri}]\nError: {request.error}\nText: {request.downloadHandler.text}");
